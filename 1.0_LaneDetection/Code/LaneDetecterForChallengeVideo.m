...
% Read video.
v = VideoReader('../Input/challenge_video.mp4');
numberOfFrames = v.NumberOfFrames;
% Open video writer, prepare for recording.
vid = VideoWriter('LaneDetectionForChallengeVideo', 'MPEG-4');
vid.FrameRate = 30;
open(vid);
...
for i = 1 : numberOfFrames
    ...
    recombinedRGBImage = cat(3, imadjustedRedChannel, imadjustedGreenChannel, imadjustedBlueChannel);
    ...
    % Eliminate the noise from the same color car on the road.
    ...
    %% Store the left and right lines' points seperately.
    leftpoints = struct([]);
    rightpoints = struct([]);
    n = length(lines);
    i = 1;
    ...
    while n > 0
        ...
        if lines(n).theta > 0 && lines(n).theta < 85 % Left line
        ...
        elseif lines(n).theta < 0 && lines(n).theta > -85 % Right line  
    ...
    if isempty(leftLineX)
        ...
        % Linear regression for the left and right line to cancel out outliers.
    ...
    if isempty(rightLineX)
        ...
        % Linear regression for the right and right line to cancel out outliers.
    ...
    if ~isempty(leftLineXPrev)
        leftLineX(1:2) = tau_slope * leftLineXPrev(1:2) + (1 - tau_slope) * leftLineX(1:2);
        pLeftValue(1:2) = tau_offset * pLeftValuePrev(1:2) + (1 - tau_offset) * pLeftValue(1:2);
    end
    if ~isempty(rightLineXPrev)
        rightLineX(1:2) = tau_slope * rightLineXPrev(1:2) + (1 - tau_slope) * rightLineX(1:2);
        pRightValue(1:2) = tau_offset * pRightValuePrev(1:2) + (1 - tau_offset) * pRightValue(1:2);
    end
    % Update the global variables for final plotting.
    leftLineXPrev = leftLineX(1:2);
    rightLineXPrev = rightLineX(1:2);
    pLeftValuePrev = pLeftValue(1:2);
    pRightValuePrev = pRightValue(1:2);
    
    %% Extend the left and right lines to the maximum area of mask1.
